...
class Employee extends Model
{
    ...
    /**
     * Get complete PDS data for PDF generation
     */
    public function getPdsDataForPdf(): array
    {
        return [
            ...
            'personal_info' => [
                ...
                'date_of_birth' => $this->birth_date?->format('m/d/Y'),
                'place_of_birth' => $this->place_of_birth,
                'sex' => $this->gender,
                'civil_status' => $this->civil_status,
                'civil_status_other_details' => $this->civil_status_other_details,
                'height' => $this->height,
                'weight' => $this->weight,
                ...
                'gsis_id' => $this->gsis_number,
                'pagibig_id' => $this->pagibig_number,
                'philhealth_id' => $this->philhealth_number,
                'sss_id' => $this->sss_number,
                'tin_id' => $this->tin_number,
                'agency_employee_no' => $this->agency_employee_no,
                'residential_address' => [
                    'house_block_lot_no' => $this->res_house_block_lot_no,
                    'street' => $this->res_street,
                    'subdivision_village' => $this->res_subdivision_village,
                    'barangay' => $this->res_barangay,
                    'city_municipality' => $this->res_city_municipality,
                    'province' => $this->res_province,
                    'zip_code' => $this->res_zip_code,
                ],
                'permanent_address' => [
                    'house_block_lot_no' => $this->perm_house_block_lot_no,
                    'street' => $this->perm_street,
                    'subdivision_village' => $this->perm_subdivision_village,
                    'barangay' => $this->perm_barangay,
                    'city_municipality' => $this->perm_city_municipality,
                    'province' => $this->perm_province,
                    'zip_code' => $this->perm_zip_code,
                ],
                'contact_info' => [
            ...
            'family_background' => [
                'spouse' => $this->familyBackground ? [
                    'surname' => $this->familyBackground->spouse_surname,
                    'first_name' => $this->familyBackground->spouse_first_name,
                    'middle_name' => $this->familyBackground->spouse_middle_name,
                    'occupation' => $this->familyBackground->spouse_occupation,
                    'employer' => $this->familyBackground->spouse_employer,
                    'business_address' => $this->familyBackground->spouse_business_address,
                    'telephone_no' => $this->familyBackground->spouse_telephone_no,
                ] : null,
                'parents' => $this->familyBackground ? [
                    'father' => [
                        'surname' => $this->familyBackground->father_surname,
                        'first_name' => $this->familyBackground->father_first_name,
                        'middle_name' => $this->familyBackground->father_middle_name,
                    ],
                    'mother' => [
                        'maiden_name' => $this->familyBackground->mother_maiden_name,
                        'surname' => $this->familyBackground->mother_surname,
                        'first_name' => $this->familyBackground->mother_first_name,
                        'middle_name' => $this->familyBackground->mother_middle_name,
                    ],
                ] : null,
                'children' => $this->children->map(function ($child) {
                    return [
                        'full_name' => $child->full_name,
                        'date_of_birth' => $child->date_of_birth?->format('m/d/Y'),
                    ];
                })->toArray(),
            ],

            // Panel 3: Educational Background
            'educational_background' => $this->education->map(function ($edu) {
                return [
                    'level' => $edu->level,
                    ...
                    'highest_level_units_earned' => $edu->highest_level_units_earned,
                    'year_graduated' => $edu->year_graduated,
                    'scholarship_honors_received' => $edu->scholarship_honors_received,
                ];
            })->toArray(),

            // Panel 4: Civil Service Eligibility
            'civil_service_eligibility' => $this->pdsEligibilities->map(function ($eligibility) {
                return [
                    'eligibility_name' => $eligibility->eligibility_name,
                    'rating' => $eligibility->rating,
                    'date_of_examination' => $eligibility->date_of_examination?->format('m/d/Y'),
            ...
            'work_experience' => $this->workExperiences->map(function ($work) {
                return [
                    'inclusive_date_from' => $work->inclusive_date_from?->format('m/d/Y'),
                    'inclusive_date_to' => $work->inclusive_date_to?->format('m/d/Y'),
            ...
            'voluntary_work' => $this->voluntaryWork->map(function ($work) {
                return [
                    'organization_name_address' => $work->organization_name_address,
                    'inclusive_date_from' => $work->inclusive_date_from?->format('m/d/Y'),
                    'inclusive_date_to' => $work->inclusive_date_to?->format('m/d/Y'),
            ...
            'learning_development' => $this->trainings->map(function ($training) {
                return [
                    'training_title' => $training->training_title,
                    'inclusive_date_from' => $training->start_date?->format('m/d/Y'),
                    'inclusive_date_to' => $training->end_date?->format('m/d/Y'),
                    'number_of_hours' => $training->number_of_hours,
                    'type_of_ld' => $training->type_of_ld,
                    'conducted_by_sponsor' => $training->conducted_by_sponsor,
                ];
            })->toArray(),

            // Panel 8: Other Information
            'other_information' => [
                'special_skills' => $this->specialSkills->map(function ($skill) {
                    return ['description' => $skill->description];
                })->toArray(),
                'distinctions' => $this->distinctions->map(function ($distinction) {
                    return ['description' => $distinction->description];
                })->toArray(),
                'memberships' => $this->memberships->map(function ($membership) {
                    return ['description' => $membership->description];
                })->toArray(),
            ],
            ...
            'questionnaire' => $this->questionnaire ? [
                'questions_answers' => $this->questionnaire->questions_answers,
                'government_service_related' => $this->questionnaire->government_service_related,
                'administrative_offense' => $this->questionnaire->administrative_offense,
                'criminal_charged' => $this->questionnaire->criminal_charged,
                'convicted' => $this->questionnaire->convicted,
                'separated_service' => $this->questionnaire->separated_service,
                'candidate_resigned' => $this->questionnaire->candidate_resigned,
                'immigrant_indigenous' => $this->questionnaire->immigrant_indigenous,
                'pwd' => $this->questionnaire->pwd,
                'solo_parent' => $this->questionnaire->solo_parent,
            ] : null,
        ];
    }
}
