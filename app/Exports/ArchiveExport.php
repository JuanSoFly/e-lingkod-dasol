...
class ArchiveExport implements FromCollection, WithHeadings, WithMapping, WithColumnWidths, WithStyles
{
    use WithExcelFormatting;

    protected $archivedEmployees;

    public function __construct($archivedEmployees)
    {
    ...
    {
        return $this->archivedEmployees->load(['user', 'archivedBy']);
    }
    ...
    public function map($employee): array
    {
        return [
            ...
            $employee->archived_at?->format('m/d/Y'),
            $employee->archivedBy?->name ?? 'System',
            $employee->can_restore ? 'Yes' : 'No',
        ];
    }

    /**
     * Custom column formatting for archive data.
     */
    protected function getColumnFormats(): array
    {
        return [
            ...
            'R' => 'mm/dd/yyyy',                     // Archived At
            'S' => NumberFormat::FORMAT_TEXT,        // Archived By
            'T' => NumberFormat::FORMAT_TEXT,        // Can Restore
        ];
    }

    /**
     * Custom column widths for archive data.
     */
